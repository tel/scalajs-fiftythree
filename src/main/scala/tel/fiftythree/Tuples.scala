...
/**
  ...
  *
  * These classes create a nice implicit mechanism for "flattening" tuples.
  * This shows up a lot when creating routes since we cannot do the standard
  * Applicative tricks but don't want to deal with the many variations on an
  * ordered set of results, tupled pairwise.
  */
private[fiftythree] object Tuples {

  private[fiftythree] trait Composition[A, B] {
    type C
    val _1: C => A
    val _2: C => B
    val smash: (A, B) => C
  ...
  private[fiftythree] trait Composition_Priority_0 {
    implicit def ***[A, B] = Composition[A, B, (A, B)](_._1, _._2, (_, _))
  }

  private[fiftythree] trait Composition_Priority_1 extends Composition_Priority_0 {
  ...
  private[fiftythree] trait Composition_Priority_2 extends Composition_Priority_1 {
    implicit def _toA[A] = Composition[Unit, A, A](_ => (), identity, (_, a) => a)
    implicit def Ato_[A] = Composition[A, Unit, A](identity, _ => (), (a, _) => a)
  }

  private[fiftythree] object Composition extends Composition_Priority_2 {
